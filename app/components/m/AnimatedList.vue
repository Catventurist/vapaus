...
const handleKeyDown = (e: KeyboardEvent) => {
  if (e.key === 'ArrowDown' || (e.key === 'Tab' && !e.shiftKey)) {
    ...
    if (selectedIndex.value >= 0 && selectedIndex.value < props.items.length) {
      e.preventDefault()
      emit('itemSelected', props.items[selectedIndex.value] as string, selectedIndex.value)
    }
  }
}
...
<template>
  <div ref="containerRef" class="relative w-[500px]" :class="props.class.trim()">
    <div
      ref="listRef" :class="`max-h-[400px] overflow-y-auto p-4 ${displayScrollbar
        ? '[&::-webkit-scrollbar]:w-[8px] [&::-webkit-scrollbar-track]:bg-[#0b0b0b] [&::-webkit-scrollbar-thumb]:bg-[#222] [&::-webkit-scrollbar-thumb]:rounded-[4px]'
        : 'scrollbar-hide'}`"
      :style="{ scrollbarWidth: displayScrollbar ? 'thin' : 'none', scrollbarColor: '#222 #0b0b0b' }" @scroll="handleScroll">
      <Motion
        v-for="(item, index) in items" :key="index" tag="div" :data-index="index" class="mb-4 cursor-pointer"
        :initial="{ scale: 0.7, opacity: 0 }"
        :animate="getItemInView(index) ? { scale: 1, opacity: 1 } : { scale: 0.7, opacity: 0 }"
        :transition="{ duration: 0.2, delay: 0.1 }" @mouseenter="() => setSelectedIndex(index)"
        @click="() => { setSelectedIndex(index); emit('itemSelected', item, index) }">
        <div :class="`p-4 bg-[#111] rounded-lg ${selectedIndex === index ? 'bg-[#222]' : ''} ${itemClass}`">
    ...
    <div
      v-if="showGradients" :style="{ opacity: topGradientOpacity }"
      class="absolute top-0 left-0 right-0 h-[50px] bg-linear-to-b from-[#0b0b0b] to-transparent pointer-events-none transition-opacity duration-300 ease" />
    <div
      v-if="showGradients" :style="{ opacity: bottomGradientOpacity }"
      class="absolute bottom-0 left-0 right-0 h-[100px] bg-linear-to-t from-[#0b0b0b] to-transparent pointer-events-none transition-opacity duration-300 ease" />
  </div>
</template>
