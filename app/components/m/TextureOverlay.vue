...
const finalOpacity = props.opacity ?? defaultOpacities[props.texture]
const pattern = texturePatterns[props.texture]
</script>

<template>
  <div v-if="texture === 'none'" />
  <div v-else class="absolute inset-0 pointer-events-none" :class="[pattern, props.class]" :style="{ opacity: finalOpacity }" />
</template>
