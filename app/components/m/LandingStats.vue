...
<template>
	<div class="mx-auto max-w-7xl px-6">
		<motion.div class="mb-16 text-center" :initial="{ opacity: 0, y: 20 }" :transition="{ duration: 0.4 }" 
		...
		<div class="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-4" ref="aref">
			<motion.div v-for="(stat, index) in stats" :key="index" :animate="isInView ? { opacity: 1, y: 0, scale: 1 } : { opacity: 0, y: 30, scale: 0.9 }"
				...
				<motion.div 
					...
					class="mb-4 text-3xl" :initial="{ rotate: -10, scale: 0.8 }"
				...
				<motion.div v-if="stat.trend" :animate="isInView ? { opacity: 1, x: 0 } : { opacity: 0, x: -10 }"
